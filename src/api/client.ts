...
export class AxonClient implements LLMClient {
	...
	private requestHeaders(): Record<string, string> {
		const headers: Record<string, string> = { [X_AXONCODE_TASKID]: this.options.taskId }
		if (this.options.organizationId) headers[X_ORGANIZATIONID] = this.options.organizationId
	...
	async *createMessage(
		...
		for await (const chunk of stream) {
			...
			if ("error" in chunk) {
				const error = (chunk as { error?: { message?: string; code?: number } }).error
				const err = new Error(`Axon API Error ${error?.code ?? ""}: ${error?.message ?? "unknown error"}`)
				;(err as { status?: number }).status = error?.code
				throw err
			}

			if ("provider" in chunk && typeof (chunk as { provider?: string }).provider === "string") {
